...
function ExternalIcon({ className = "flex-shrink-0" }: { className?: string }) {
  return (
      ...
      <div className="flex flex-col gap-4">
        ...
        {PRESS_HIGHLIGHTS.map((item) => (
          <a
            ...
            className="group flex flex-col gap-2 p-4 bg-white border border-[#E5E5E5] rounded-xl hover:border-[#12ff8060] hover:bg-[#12ff800a] hover:shadow-[0_0_0_1px_#12ff8020] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#12FF80] relative"
          >
            {/* Green accent bar */}
            <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#12FF80] rounded-l-xl opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
            <div className="flex items-start justify-between gap-2">
              <span className="text-base font-semibold text-[#1A1A1A] leading-snug group-hover:text-[#12FF80] transition-colors">
                {item.headline}
              </span>
              <ExternalIcon className="flex-shrink-0 mt-0.5 text-[#1A1A1A66] group-hover:text-[#12FF80] transition-colors" />
            </div>
            <span className="text-sm text-[#1A1A1A99]">{item.summary}</span>
          </a>
        ))}
      </div>
